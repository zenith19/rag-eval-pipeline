...
The demo corpus is a set of NLP research papers (coreference resolution and low-resource
language modeling) — the kind of literature review this tool was built to make searchable.
Documents are stored in **AWS S3**, embedded locally and indexed in **Qdrant** (run as a
Docker container), and retrieved by a single shared retriever used by both the evaluation
harness (offline) and a **FastAPI** service (online). Answers are generated by
**Amazon Bedrock**.
